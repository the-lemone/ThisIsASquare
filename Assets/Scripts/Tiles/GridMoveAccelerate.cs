...
public class GridMoveAccelerate : GridBase
{
    ...
    private void HandleInput()
    {
        ...
        if (inputDirection != Vector3.zero)
        {
            ...
            if (targetTile && _cooldown == 0)
            {
                ...
                float limitCd = Mathf.Clamp(cooldown, 0.01f, 0.5f);
                float pitchLimit = Mathf.Clamp(1f + _timesMoved * 0.05f, 1f, 2f);
                _cooldown = limitCd;
                pitch = pitchLimit;
